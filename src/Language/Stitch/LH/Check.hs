...
check
  :: Globals
  -> ClosedUExp
  -> (e : WellTypedExp [] -> { t: Ty | exprType e = t } -> Either TyError b)
  -> Either TyError b
@-}
  ...
  where
    ...
    go ctx ue f = case ue of
      ...
      UGlobal name -> case lookupGlobal name globals of
        Just (TypedExp e t) -> f (e ? aClosedExpIsValidInAnyContext [] ctx e) t
        Nothing -> Left (OutOfScopeGlobal name)

      -- XXX: Using $ here causes liquid haskell to crash
      ULam ty body -> go (ty:ctx) body (\r rty -> f (Lam ty r) (TFun ty rty))
      ...
      ULet e1 e2 ->
        go ctx e1 (\te1 ty1 -> go (ty1:ctx) e2 (\te2 ty2 ->
          f (Let te1 te2) ty2
        ))
